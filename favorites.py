...
favs = f.display_favorites(user_id)
...
if not favs.empty:
    ...
    for index, row in favs.iterrows():
        ...
        if toggle:
            f.toggle_notif(user_id, row['dish_id'])
            st.rerun()
        
        col2.write(dl.get_dish_name(row["dish_id"]))
    ...
    faves = f.get_faves_for_week(user_id, date.today())
    ...
    if faves:
        ...
        for fave in faves:
            col1, col2, col3, col4 = st.columns((1.75,0.75,0.75,0.75))
            col1.write(fave)
            for fav in faves[fave]:
                    
                col2.write(fav["meal"])
                col3.write(fav["location"])
                col4.write("/".join(fav["date"].split("-")[1:]))
            st.write("---")
    else:
    ...
    with col2.container():
                    st.bar_chart(v.hall_popularity_last_7_days(), 
                     horizontal=True, 
                     height=240,
                     width=250,
